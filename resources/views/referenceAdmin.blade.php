...
<div class="logo">
    ...
    <div class="card " style="padding: 2%">
      <form method="get" action="<?php echo url('/reference'); ?>">
      Choisissez marque : <select name="marqueid">
      <option value="0">...</option>
          <?php 
              for($i=0;$i<count($marque);$i++){ ?>
                  <option value="<?php echo ($marque[$i]->id) ?>"><?php echo ($marque[$i]->intitule) ?></option> <?php
              }
          ?>
          </select>
        ...
        <?php 
            for($i=0;$i<count($liste);$i++){?>
              <tr>
                  ...
                  <td><?php echo($liste[$i]->getMarque()->intitule);?></td>
                  <td><?php echo($liste[$i]->getString());?></td>
                  <td><?php echo($liste[$i]->ram);?>Go</td>
                  <td><?php echo($liste[$i]->ecran);?>''</td>
                  <td><?php echo($liste[$i]->disquedur);?>Go</td>
                  <td><?php echo($liste[$i]->prixachat);?>Ar</td>
      ...
      <div class="modal modal-search fade" id="sup{{ $i }}" tabindex="-1" role="dialog" aria-labelledby="nouveau" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            Etes-vous sur de supprimer cette reference de laptop?
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <i class="tim-icons icon-simple-remove"></i>
              </button>
            </div>
            <a href="/suppressionReference?id=<?php echo ($liste[$i]->id)?>"><button class="btn btn-danger" >OUI</button></a>
          </div>
        </div>
      </div>
              </tr>
    ...
    <div class="modal modal-search fade" id="nouveau" tabindex="-1" role="dialog" aria-labelledby="nouveau" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <form method="get"  action="<?php echo url('/insertionReference'); ?>" >
            Choisissez la marque: <select class="form-control" name="idmarque">
                    <?php 
                        for($i=0;$i<count($marque);$i++){ ?>
                            <option value="<?php echo ($marque[$i]->id) ?>"><?php echo ($marque[$i]->intitule) ?></option> <?php
                        }
            ...
            Choisissez le processeur: <select class="form-control" name="idprocesseur">
            <?php 
            ...
            ?>
             ...
             Entrez intitule :        
            <input name="intitule" type="text" required/><br>
            Entrez ram :        
            <input name="ram" type="number" required/><br>
            Entrez ecran :        
            <input name="ecran" type="number" required/><br>
            Entrez capacite disque dur :        
            <input name="disquedur" type="number" required/><br>  
            Entrez le prix d'achat :        
            <input name="prixachat" type="number" required/><br>  
