...
<head>
<?php 
  $liste=$data['processeur'];
  $type=$data['typeprocesseur'];
?>
...
<div class="logo">
        ...
        <?php 
            for($i=0;$i<count($liste);$i++){?>
              <tr>
                  ...
                  <td><?php echo($liste[$i]->getTypeProcesseur()->intitule." ".$liste[$i]->generation."e generation");?></td>
      ...
      <div class="modal modal-search fade" id="sup{{ $i }}" tabindex="-1" role="dialog" aria-labelledby="nouveau" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            Etes-vous sur de supprimer ce modele de processeur?
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <i class="tim-icons icon-simple-remove"></i>
              </button>
            </div>
            <a href="/suppressionProcesseur?id=<?php echo ($liste[$i]->id)?>"><button class="btn btn-danger" >OUI</button></a>
          </div>
        </div>
      </div>
              </tr>
    ...
    <div class="modal modal-search fade" id="nouveau" tabindex="-1" role="dialog" aria-labelledby="nouveau" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
            Insertion Processeur
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <i class="tim-icons icon-simple-remove"></i>
              </button>
            </div>
            <form method="get"  action="<?php echo url('/insertionProcesseur'); ?>" >
            Choisissez : <select class="form-control" name="idtypeprocesseur">
                    <?php 
                        for($i=0;$i<count($type);$i++){ ?>
                            <option value="<?php echo ($type[$i]->id) ?>"><?php echo ($type[$i]->intitule) ?></option> <?php
                        }
                    ?>
                    </select><br>
             Entrez generation :        
                    <input name="generation" type="number" required/>
