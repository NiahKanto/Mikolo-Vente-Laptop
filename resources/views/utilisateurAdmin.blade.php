    ...
    <nav class="navbar navbar-expand-lg navbar-absolute navbar-transparent">
        <div class="container-fluid">
          <div class="navbar-wrapper">
            ...
            <a class="navbar-brand" href="javascript:void(0)">Park'in</a>
          </div>
        ...
        <?php 
            for($i=0;$i<count($liste);$i++){?>
              <tr>
                  ...
                  <td><button class="btn btn-info" data-toggle="modal" data-target="#modif" onClick="setId('<?php echo ($liste[$i]->id) ?>')" ><i class="tim-icons icon-send"></i></button></td>
      ...
      <div class="modal modal-search fade" id="sup{{ $i }}" tabindex="-1" role="dialog" aria-labelledby="nouveau" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <a href="/suppressionUtilisateur?id=<?php echo ($liste[$i]->id)?>"><button class="btn btn-danger" >OUI</button></a>
          </div>
        </div>
      </div>
              </tr>
    ...
    <div class="modal modal-search fade" id="nouveau" tabindex="-1" role="dialog" aria-labelledby="nouveau" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <form method="get"  action="<?php echo url('/insertionUtilisateur'); ?>" >
                    Entrez le nom :        
                    <input name="nom" type="text" required/><br>
                    Entrez le pseudo :        
                    <input name="pseudo" type="text" required/><br>
                    Entrez le mot de passe :        
                    <input name="mdp" type="text" required/><br>
                    Choisissez le point de vente: <select class="form-control" name="idpointvente">
                    <?php 
                        for($i=0;$i<count($pointvente);$i++){ ?>
                            <option value="<?php echo ($pointvente[$i]->id) ?>"><?php echo ($pointvente[$i]->lieu) ?></option> <?php
                        }
                    ?>
                    </select><br>
      ...
      <div class="modal modal-search fade" id="modif" tabindex="-1" role="dialog" aria-labelledby="modif" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <form method="get" action="<?php echo url('/modifierUtilisateur'); ?>" >
            <input hidden name="idUtilisateur" id="utilisateur" type="text" required/>
            Choisissez le point de vente: <select class="form-control" name="idpointvente">
              <?php 
                  for($i=0;$i<count($pointvente);$i++){ ?>
                      <option value="<?php echo ($pointvente[$i]->id) ?>"><?php echo ($pointvente[$i]->lieu) ?></option> <?php
            ...
            <button class="btn" type="submit" >Deplacer</button>
            </form>
          </div>
        </div>
      </div>
...
<script src="assets/js/core/jquery.min.js"></script>
  ...
  <script src="assets/demo/demo.js"></script>
    ...
    <script type="text/javascript">
        ...
        function setId(id){
            val1=document.getElementById("utilisateur");
            val1.value=id;
        }
    </script>
</body>
